...
async function fetchStockData() {
    ...
    try {
        ...
        if (!polygonApiKey) {
            loadingArea.innerText = 'Missing Polygon API key. Add VITE_POLYGON_API_KEY to your .env file.'
            return
        }
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            ...
            if (response.ok) {
                ...
                loadingArea.innerText = `There was an error fetching stock data (${response.status}).`
                return ''
            }
        }))
...
async function fetchReport(data) {
    ...
    try {
        ...
        if (!openAiKey) {
            loadingArea.innerText = 'Missing OpenAI API key. Add VITE_OPENAI_API_KEY to your .env file.'
            return
        }
